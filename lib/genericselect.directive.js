...
leodido.directive.GenericSelect.prototype.build = function (tElement, tAttrs) {
  goog.DEBUG && console.log('GenericSelectDirective::compile');
  // Force 'data-name' attribute (if it exists, also if empty) to be 'name' attribute
  if (angular.isDefined(tElement.attr('data-name'))) {
    tElement.attr('name', tElement.attr('data-name'));
    tAttrs.$attr.name = 'name'; // Needed to support interpolation that comes ...
